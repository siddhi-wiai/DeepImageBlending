...
os.makedirs(config["output_data"]["images"], exist_ok = True)
os.makedirs(config["output_data"]["labels"], exist_ok = True)
os.makedirs(config["output_data"]["metadata"], exist_ok = True)


source_folder = config["input_data"]["foregroundImages"]
mask_folder = config["input_data"]["foregroundMasks"]
target_folder = config["input_data"]["backgroundImages"]
outputImages_folder = config["output_data"]["images"]
outputLabels_folder = config["output_data"]["labels"]
outputMetadata_folder = config["output_data"]["metadata"]
...
source_files = natsort.natsorted(glob.glob(source_folder + "*.jpg"))[:20]
mask_files = natsort.natsorted(glob.glob(mask_folder + "*.png"))[:20]
target_files = natsort.natsorted(glob.glob(target_folder + "*.jpg"))[:40]

assert (len(mask_files) == len(source_files)), "Check the input data folder: there is mismatch in no of foreground images and masks!!"

# np.random.seed(0)
# np.random.shuffle(target_files)

#print(len(source_files), len(mask_files), len(target_files), sep="\n")


for i in range(3): #remove the har-coded 3 

    np.random.seed(i * 10)
    np.random.shuffle(target_files)
    target_files = target_files[: len(source_files)]
    ...
    for source_file, mask_file, target_file in zip(source_files, mask_files, target_files):
        ...
        centers= np.random.randint(low = ss, high = ts - ss, size = 2) # Remove the hard-coded number 3
        

        # to do:
        # sample 3 different locations and set x_start, y_start
        # set np.random.seed and shuffle target files - use 3 different seeds
        # in loop of 3, run the following 
        # also check pest size param (fixed or random?)
        # filename to store -> source image_target image ...also store labels/ *.txt file with bound box as per location
        # also store a folder with metadata -> source file, background file, 
        ...
        #file_name = re.search(r'\d+', source_file).group(0)
        source_file_name = os.path.splitext(os.path.basename(source_file))[0]
        pest_class_id = int(source_file_name[2:5])
        ...
        if num_steps1 > 0:
            ...
            first_pass_img_file = os.path.join(outputImages_folder, f'{source_file_name}_{i}.png')
            imsave(first_pass_img_file, blend_img_np.astype(np.uint8))

            str_metadata = f"{source_file_name}_{i}.png {source_file} {mask_file} {target_file} {(y_start, x_start)}\n" #interchanged x and y start

            metadata_path = os.path.join(outputMetadata_folder, "metadata.txt")
            with open(metadata_path, 'a') as f:
                f.write(str_metadata)
                f.close()

            label_path = os.path.join(outputLabels_folder, f'{source_file_name}_{i}.txt')
            with open(label_path, 'a') as f:
                f.write(f"{pest_class_id} {y_start/ts} {x_start/ts} {ss/ts} {ss/ts}") #interchanged x and y start
        ...
        if num_steps2 > 0:
            ...
            imsave(os.path.join(outputImages_folder, f'{source_file_name}_second_pass_{i}.png'), input_img_np.astype(np.uint8))
